...
void main() async {
  ...
  // ------------------------------------------------------------
  // GLOBAL SERVICES
  // These are lightweight registrations/configurations.
  // ------------------------------------------------------------
  GlobalDeeplinkHandler.init();
  ...
  try {
    ...
    if (kDebugMode) {
      debugPrint('⚠️ GlobalDeeplinkHandler registration failed: $e');
    }
  }

  // Crash prevention registration.
  CrashPrevention();
  ...
  // Register only. Heavy initialization happens later.
  // ------------------------------------------------------------
  try {
    if (!Get.isRegistered<MartFirestoreService>()) {
      Get.put(MartFirestoreService(), permanent: true);
    }
  } catch (e, stackTrace) {
    if (kDebugMode) {
      debugPrint('❌ Failed to register MartFirestoreService: $e');
      debugPrintStack(stackTrace: stackTrace);
    }
  }
...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  bool _reviewCheckScheduled = false;
  bool _paymentRecoveryRunning = false;
  ...
  Future<void> _recoverPendingPayment() async {
    ...
    try {
      ...
      await cartController.checkPendingPaymentAndPlaceOrder();
    } catch (e, stackTrace) {
      if (kDebugMode) {
        debugPrint('⚠️ [APP_LIFECYCLE] Pending payment recovery failed: $e');
        debugPrintStack(stackTrace: stackTrace);
      }
    } finally {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      // This handles the case where user closes app immediately after payment
      WidgetsBinding.instance.addPostFrameCallback((_) {
        unawaited(_recoverPendingPayment());
      });
      _scheduleReviewCheck();
    } else if (state == AppLifecycleState.paused) {
      // Notify deep link service that app has paused
