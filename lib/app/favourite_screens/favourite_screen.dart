...
class _FavouriteScreenState extends State<FavouriteScreen> {
  // Cache for product prices to avoid repeated calculations
  final Map<String, Map<String, dynamic>> _priceCache = {};

  // Cache for vendor models
  final Map<String, VendorModel> _vendorCache = {};

  // Debounce timer for tab switching
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              Expanded(
                child: InkWell(
                  onTap: () => _handleTabChange(favouriteProvider, true),
                  child: Container(
                    decoration: favouriteProvider.favouriteRestaurant == false
                        ? null
                        : ShapeDecoration(
              ...
              Expanded(
                child: InkWell(
                  onTap: () => _handleTabChange(favouriteProvider, false),
                  child: Container(
                    decoration: favouriteProvider.favouriteRestaurant == true
                        ? null
                        : ShapeDecoration(
  ...
  void _handleTabChange(FavouriteProvider provider, bool value) {
    // Debounce tab switching to prevent rapid rebuilds
    _debounceTimer?.cancel();
    _debounceTimer = Timer(const Duration(milliseconds: 200), () {
      provider.changeTabUpdate(value);
    });
  }
  ...
  Widget _buildRestaurantItem(
    ...
    // Cache vendor model
    if (!_vendorCache.containsKey(vendorModel.id)) {
      _vendorCache[vendorModel.id!] = vendorModel;
    }

    return InkWell(
      onTap: () => _handleRestaurantTap(vendorModel, context),
      child: Padding(
        padding: const EdgeInsets.only(bottom: 20),
        child: Container(
          decoration: ShapeDecoration(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  Transform.translate(
                    ...
                    child: Row(
                      ...
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
                        _buildDeliveryBadge(vendorModel),
                        _buildRatingBadge(vendorModel),
                        const SizedBox(width: 6),
                        _buildDistanceBadge(vendorModel),
                      ],
                    ),
                  ),
                ],
              ...
              const SizedBox(height: 15),
              _buildRestaurantInfo(vendorModel),
              const SizedBox(height: 10),
            ],
          ),
        ),
  ...
  void _handleRestaurantTap(
    ...
    if (vendorModel.zoneId == Constant.selectedZone!.id) {
      ...
      final cachedVendor = _vendorCache[vendorModel.id] ?? vendorModel;
      restaurantDetailsProvider.initFunction(vendorModels: cachedVendor);
      Get.to(
        const RestaurantDetailsScreen(),
        arguments: {"vendorModel": cachedVendor},
      );
    } else {
      ShowToastDialog.showToast(
        "Sorry, The Zone is not available in your area. change the other location first."
  ...
  Widget _buildDeliveryBadge(VendorModel vendorModel) {
  ...
  Widget _buildRatingBadge(VendorModel vendorModel) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
      decoration: ShapeDecoration(
        color: AppThemeData.primary50,
  ...
  Widget _buildDistanceBadge(VendorModel vendorModel) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
      decoration: ShapeDecoration(
        color: AppThemeData.secondary50,
  ...
  Widget _buildRestaurantInfo(VendorModel vendorModel) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Future<void> _removeFavoriteRestaurant(
    ...
    try {
      await favouriteProvider.removeFavoriteRestaurantUI(
        vendorModel.id.toString(),
        index,
      );
      // Remove from cache
      _vendorCache.remove(vendorModel.id);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to remove: Please try again'.tr),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  Widget _buildFoodItem(
    ...
    if (_priceCache.containsKey(productId)) {
      return _buildFoodItemContent(
        productModel,
        favouriteProvider,
        index,
        context,
        _priceCache[productId]!,
      );
    }
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return _buildFoodItemSkeleton();
        } else if (snapshot.hasError) {
          return _buildFoodItemError(snapshot.error.toString());
        } else if (snapshot.data != null) {
          // Cache the price data
          _priceCache[productId] = snapshot.data!;
          return _buildFoodItemContent(
            productModel,
            favouriteProvider,
            index,
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Container(height: 20, width: 80, color: AppThemeData.grey200),
              ],
            ),
          ),
        ],
  ...
  Widget _buildFoodItemError(String error) {
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 8),
      padding: const EdgeInsets.all(16),
      decoration: ShapeDecoration(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Error loading item',
              style: TextStyle(color: AppThemeData.grey700),
            ),
          ),
        ],
      ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    _buildFoodRating(productModel),
                    const SizedBox(height: 4),
                    _buildFoodDescription(productModel),
                    const SizedBox(height: 8),
                    _buildFoodPrice(price, disPrice),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildFoodImage(ProductModel productModel) {
    return Stack(
      children: [
        ClipRRect(
          borderRadius: BorderRadius.circular(12),
        ...
        Positioned(
          ...
          child: Container(
            ...
            child: productModel.nonveg == true
                ? SvgPicture.asset("assets/icons/ic_nonveg.svg", height: 12)
                : SvgPicture.asset("assets/icons/ic_veg.svg", height: 12),
          ),
        ),
      ],
    );
  ...
  Widget _buildFavoriteButton(
    ...
    return GestureDetector(
      onTap: () =>
          _removeFavoriteFood(productModel, favouriteProvider, index, context),
      child: Padding(
        padding: const EdgeInsets.all(4),
        child: SvgPicture.asset(
          "assets/icons/ic_like_fill.svg",
  ...
  Widget _buildFoodRating(ProductModel productModel) {
    return Row(
      children: [
        SvgPicture.asset(
          "assets/icons/ic_star.svg",
        ...
        Text(
          Constant.calculateReview(
            reviewCount: productModel.reviewsCount!.toStringAsFixed(0),
  ...
  Widget _buildFoodDescription(ProductModel productModel) {
    return Text(
      productModel.description ?? "",
      style: TextStyle(
        color: AppThemeData.grey600,
  ...
  Widget _buildFoodPrice(String price, String disPrice) {
    final hasDiscount =
        double.parse(disPrice) > 0 &&
      ...
      children: [
        ...
        Text(
          ...
          style: TextStyle(
            ...
            color: hasDiscount ? AppThemeData.grey500 : AppThemeData.primary300,
            fontFamily: AppThemeData.bold,
            fontWeight: FontWeight.w700,
  ...
  Future<void> _removeFavoriteFood(
    ProductModel productModel,
    FavouriteProvider favouriteProvider,
    int index,
    BuildContext context,
    ...
    try {
      await favouriteProvider.removeFavoriteFoodUI(
        productModel.id.toString(),
        index,
      );
      // Clear cache for this item
      _priceCache.remove(productModel.id.toString());
    ...
    try {
      ...
      if (attrs.isNotEmpty) {
        ...
        for (var i = 0; i < attrs.length; i++) {
          ...
          if (element.attributeOptions != null &&
              element.attributeOptions!.isNotEmpty) {
            selectedVariants.add(element.attributeOptions!.first);
          }
        }
      ...
      final result = {'price': price, 'disPrice': disPrice};

      // Cache the result
      _priceCache[productId] = result;
