// import 'package:jippymart_customer/app/DealsScreen/widgets/appbar.dart';
...
//   final latitude = Constant.selectedZone?.latitude ?? 0.0;
//   final longitude = Constant.selectedZone?.longitude ?? 0.0;
//
//   int _retryCount = 0;
...
//
//     _zonePollTimer = Timer(const Duration(seconds: 1), () {
//       if (!mounted) return;
//
...
//     }
//
//     // Don't refresh if the data was loaded recently.
...
//
//     final banners = _homeProvider!.bannerBottomModel;
//
//     if (!listEquals(_dealsBanners, banners)) {
//       setState(() {
//         _dealsBanners = List<BannerModel>.from(banners);
//       });
//     }
//
//     _checkZoneChanged();
...
//
//     if (newId == null || newId.isEmpty || newId == _currentZoneId) {
//       return;
//     }
//
//     _zoneDebounce?.cancel();
...
//
//       if (confirmed == null || confirmed.isEmpty || confirmed != newId) {
//         return;
//       }
//
...
//
//     final hasCachedData =
//         _promotionsList.isNotEmpty || _dealsBanners.isNotEmpty;
//
//     if (_lastLoadTime != null &&
...
//       await _loadAllData();
//     } catch (e) {
//       debugPrint('[DEALS] Refresh error: $e');
//
//       if (mounted) {
//         setState(() {
//           _isLoading = false;
...
//     if (_isFetching) {
//       debugPrint('[DEALS] Already fetching. Skipping request.');
//       return;
//     }
//
...
//
//     if (zoneId == null || zoneId.isEmpty) {
//       debugPrint('[DEALS] Zone ID missing');
//       return;
//     }
//
//     _isFetching = true;
...
//
//     try {
//       debugPrint('[DEALS] Loading data for zone: $zoneId');
//
//       await _loadPromotions();
//       _lastLoadTime = DateTime.now();
//
//       debugPrint('[DEALS] Banners: ${_dealsBanners.length}');
//
//       debugPrint('[DEALS] Promotions: ${_promotionsList.length}');
//     } catch (e) {
...
//   //
//   //   // Prevent unnecessary second API call
//   //   if (_hasLoadedBanners && !forceRefresh) {
//   //     debugPrint('[BANNER] Already loaded -> USING EXISTING DATA');
//   //
//   //     debugPrint('[BANNER] Main: ${bannerModel.length}');
//   //
//   //     debugPrint(
//   //       '[BANNER] Best: '
...
//   //       debugPrint('[BANNER] ❌ Invalid location');
//   //       return;
//   //     }
//   //
//   //     final cacheKey =
...
//   //
//   //     debugPrint('[BANNER] Cache key: $cacheKey');
//   //
//   //     // ---------------------------------------------------------
//   //     // CACHE
//   //     // ---------------------------------------------------------
...
//   //     );
//   //
//   //     debugPrint('[BANNER] API RESPONSE RECEIVED');
//   //
//   //     debugPrint(
//   //       '[BANNER] Main: '
//   //       '${response.mainBannerInfoDtos.length}',
//   //     );
//   //
//   //     debugPrint(
//   //       '[BANNER] Best: '
//   //       '${response.bestRestaurantBannerInfoDtos.length}',
//   //     );
//   //
//   //     debugPrint(
//   //       '[BANNER] Deals: '
//   //       '${response.dealsBannerInfoDtos.length}',
//   //     );
//   //
//   //     // ---------------------------------------------------------
//   //     // CACHE RESPONSE
...
//   //     notifyListeners();
//   //
//   //     debugPrint('[BANNER] ✅ Banner data loaded successfully');
//   //   } catch (e, stackTrace) {
//   //     debugPrint('[BANNER] ❌ ERROR: $e');
//   //
//   //     debugPrint('[BANNER] STACK: $stackTrace');
//   //   } finally {
...
//   //   try {
//   //     final url = Uri.parse(
//   //       '${AppConst.outletBaseUrl}'
//   //       'fm/banners/getActiveBanners'
//   //       // '?zone_id=${Uri.encodeComponent(zoneId)}',
//   //       '?lat=17.499877&lng=78.3902145',
//   //     );
//   //
...
//   //
//   //     debugPrint('[BANNER] STATUS: ${response.statusCode}');
...
//
//     try {
//       // Temporary unique key to bypass old cache
//       final key = 'promotions_${z}_${DateTime.now().millisecondsSinceEpoch}';
//
//       debugPrint('[PROMOTIONS] Zone: $z');
//       debugPrint('[PROMOTIONS] Cache key: $key');
//
...
//       );
//
//       debugPrint('[PROMOTIONS] RAW COUNT: ${data.length}');
//
//       debugPrint('[PROMOTIONS] RAW DATA: $data');
//
//       // ------------------------------------------------------------
//       // NO DATA
//       // ------------------------------------------------------------
//
//       if (data.isEmpty) {
//         debugPrint('[PROMOTIONS] ❌ Firestore/API returned 0 promotions');
//
//         if (mounted) {
//           setState(() {
//             _promotionsList = [];
...
//           .toList();
//
//       debugPrint('[PROMOTIONS] PARSED COUNT: ${parsed.length}');
//
//       // ------------------------------------------------------------
//       // DEBUG EACH PROMOTION
//       // ------------------------------------------------------------
//
...
//
//       // ------------------------------------------------------------
//       // TEMPORARILY SHOW ALL PARSED PROMOTIONS
//       // Don't filter isAvailable yet.
//       // ------------------------------------------------------------
//
//       final promos = parsed;
//
//       debugPrint('[PROMOTIONS] AFTER FILTER COUNT: ${promos.length}');
//
//       // ------------------------------------------------------------
//       // GET RESTAURANT IDS
//       // ------------------------------------------------------------
...
//           .toList();
//
//       debugPrint('[PROMOTIONS] Vendor IDs: $vendorIds');
//
//       // ------------------------------------------------------------
//       // LOAD RESTAURANTS
//       // ------------------------------------------------------------
...
//       if (mounted) {
//         setState(() {
//           _promotionsList = sorted;
//         });
//       }
//
//       debugPrint('[PROMOTIONS] ✅ FINAL DEAL COUNT: ${sorted.length}');
//     } catch (e, stackTrace) {
...
//   @override
//   Widget build(BuildContext context) {
//     final rs = ResponsiveExtension(context).rs;
//     _cachedRS = rs;
//
...
//               child: FadeTransition(opacity: _headerFade, child: DealsAppBar()),
//             ),
//
//             // ── Category Filter Row ──────────────────────────────────────
//             // _CategoryRow(
...
//                       color: C.brand,
//                       child: (_promotionsList.isEmpty && _dealsBanners.isEmpty)
//                           ? const _EmptyState()
//                           : _ContentList(
//                               rs: rs,
...
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:jippymart_customer/app/DealsScreen/widgets/ContentList.dart';
import 'package:jippymart_customer/app/DealsScreen/widgets/DealsEmptyState.dart';
import 'package:jippymart_customer/app/DealsScreen/widgets/DealsSkeletonBody.dart';
import 'package:provider/provider.dart';
...
class _DealsScreenState extends State<DealsScreen>
  ...
  Future<void> _refreshOnResume() async {
    ...
    // Skip refresh if data was loaded very recently.
    if (_lastLoadTime != null &&
        DateTime.now().difference(_lastLoadTime!) <
            const Duration(minutes: 3)) {
      _checkZoneChanged();
  ...
  void _onHomeProviderUpdate() {
    ...
    final banners = _homeProvider!.bannerBottomModel;
    if (!listEquals(_dealsBanners, banners)) {
      setState(() => _dealsBanners = List<BannerModel>.from(banners));
    }
    ...
    await Future.wait(
      ids.where((id) => !_vendorCache.containsKey(id)).map((id) async {
        try {
          ...
          // Vendor lookup failures are non-fatal — the card falls back
          // to promotion-only data and treats the restaurant as open.
        }
      }),
    );
  }
  ...
  Future<List<PromotionModel>> _sortByOpenStatus(
    ...
    try {
      return promos..sort((a, b) {
        final openA = _isVendorOpen(a.restaurantId);
        final openB = _isVendorOpen(b.restaurantId);
        if (openA == openB) return 0;
        return openA ? -1 : 1;
      });
    } catch (_) {
  ...
  bool _isVendorOpen(String restaurantId) {
    if (_restaurantStatusCache.containsKey(restaurantId)) {
      return _restaurantStatusCache[restaurantId]!;
    }
