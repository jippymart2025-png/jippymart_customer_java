    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        if (streetName.isNotEmpty && streetName != doorNumber) {
          ...
          print(
            '[GPS_LOCATION] No street name found (door number: $doorNumber)',
          );
        }
        ...
        if (address.isEmpty || address.split(',').length < 3) {
          if (place.name != null && place.name!.isNotEmpty) {
            ...
            if (place.administrativeArea != null &&
                place.administrativeArea!.isNotEmpty &&
                !address.contains(place.administrativeArea!)) {
              address += ', ${place.administrativeArea!}';
            }
            if (place.country != null &&
                place.country!.isNotEmpty &&
                !address.contains(place.country!)) {
              address += ', ${place.country!}';
            }
          }
        }
  ...
  static Future<Map<String, double>?> getLocationForZoneDetection() async {
    try {
      ...
      if (position != null) {
        ...
        print(
          '[GPS_LOCATION] Location for zone detection: ${location['latitude']}, ${location['longitude']}',
        );
        return location;
      } else {
