  ...
  void _collectAllResults(
    ...
    if (node.isEnd && node.dataRefs.isNotEmpty) {
      for (var data in node.dataRefs) {
        ...
        results.add({'data': data, 'score': node.relevanceScore});
      }
    }
  ...
  List<String> getSuggestions(String prefix, {int maxSuggestions = 10}) {
    try {
      ...
      for (var char in lowerPrefix.split('')) {
        if (!node.children.containsKey(char)) {
          print(
            "🔍 Trie: Prefix '$prefix' not found - no children for '$char'",
          );
          return []; // Prefix not found
        }
        node = node.children[char]!;
      ...
      print(
        "🔍 Trie: Found ${suggestions.length} suggestions for '$prefix': $suggestions",
      );
      return suggestions;
    } catch (e) {
      return [];
