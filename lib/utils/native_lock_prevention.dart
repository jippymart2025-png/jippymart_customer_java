...
class NativeLockPrevention {
        ...
        return await nativeOperation().timeout(
          ...
          onTimeout: () {
            ...
            throw TimeoutException(
              'Native operation timed out: $operationName',
            );
          },
        );
      }
  ...
  static void startLockContentionMonitoring() {
    ...
    _isMonitoring = true;
    _lockContentionMonitor = Timer.periodic(const Duration(seconds: 1), (
      timer,
    ) {
    ...
    FirebaseCrashlytics.instance.log(
      'NATIVE_LOCK_PREVENTION: Started lock contention monitoring',
    );
  }
  ...
  static void _reportNativeLockContention(
    String operationName,
    Duration duration,
  ) {
    final message =
        'NATIVE_LOCK_CONTENTION: $operationName blocked for ${duration.inMilliseconds}ms';
    log(message);
  ...
  static void _reportNativeIsolateError(String operationName, dynamic error) {
    final message =
        'NATIVE_ISOLATE_ERROR: $operationName failed in isolate - $error';
    log(message);
